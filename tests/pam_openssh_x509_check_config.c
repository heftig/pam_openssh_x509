...
#include "../src/pam_openssh_x509_config.h"
...
make_config_suite(void)
{
    ...
    tcase_add_loop_test(tc_all, test_init_and_parse_config, 0, length_iapc_lt);
    tcase_add_test(tc_all, test_init_and_parse_config_params);
    tcase_add_test(tc_all, test_release_config);
